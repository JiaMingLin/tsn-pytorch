...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, BasicBlock) or isinstance(m, DistillerBasicBlock):
                    nn.init.constant_(m.bn2.weight, 0)  # type: ignore[arg-type]

    def _make_layer(self, block: Type[Union[DistillerBasicBlock, BasicBlock]], planes: int, blocks: int,
