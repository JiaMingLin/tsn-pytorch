...
class ClassifierCompressor(object):
    ...
    def load_datasets(self):
        ...
        if not all((self.train_loader, self.val_loader, self.test_loader)):
            self.train_loader, self.val_loader, self.test_loader = load_data(self.args, self.model)
...
def _init_learner(args):
    ...
    model = create_model(args)
        ...
        # args.pretrained, args.dataset, args.arch, args.num_classes,
...
def load_data(args, model, fixed_subset=False, sequential=False, load_train=True, load_val=True, load_test=True):
...
def acts_quant_stats_collection(model, criterion, loggers, args, test_loader=None, save_to_file=False):
    ...
    if test_loader is None:
        ...
        test_loader = load_data(tmp_args, model, fixed_subset=True, load_train=False, load_val=False)
...
def acts_histogram_collection(model, criterion, loggers, args):
    ...
    test_loader = load_data(args, model, fixed_subset=True, load_train=False, load_val=False)
    test_fn = partial(test, test_loader=test_loader, criterion=criterion,
                      loggers=loggers, args=args, activations_collectors=None)
